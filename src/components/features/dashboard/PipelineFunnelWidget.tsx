  ...
  return (
    <section className={cn(WIDGET_CARD, "flex flex-col", className)}>
      <div className="mb-2 shrink-0">
        ...
        <p className="text-[11px] text-white/35 md:text-[10px]">
      ...
      <div className="relative">
        <div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 right-0 z-10 border-r border-dashed border-white/5"
        />
        ...
        <div className="flex flex-col gap-3">
          {stages.map((stage) => {
            ...
            const fillClass =
              STAGE_FILL[stage.stage] ?? "bg-white/10";
              ...
              <div key={stage.stage} className="group relative w-full">
                <div className="mb-1.5 flex items-end justify-between gap-3">
                  <span className="flex min-w-0 items-center gap-2 text-sm font-medium text-white/90">
                    ...
                    <span className="shrink-0 rounded bg-white/5 px-1.5 py-0.5 text-[11px] text-white/40 md:text-[10px]">
                      {stage.dealCount}
                    </span>
                  </span>
                  <span className="shrink-0 text-sm font-bold tracking-tight text-[#b38c56]">
                    {formatCompactTRY(stage.volumeTL)}
                  </span>
                </div>

                <div className="h-2 w-full overflow-hidden rounded-full border border-white/5 bg-[#09090b] shadow-inner">
                  <div
                    className={cn(
