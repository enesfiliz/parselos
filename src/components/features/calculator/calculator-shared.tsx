...
export const CALC_CARD =
  "relative overflow-hidden rounded-2xl border border-white/5 bg-[#151f23] p-6";

export const CALC_INPUT =
  "w-full rounded-lg border border-white/5 bg-[#09090b] px-4 py-2.5 text-sm text-white transition-colors outline-none focus:border-[#b38c56]";

export const CALC_LABEL = "mb-1.5 block text-xs font-medium text-white/45";
  ...
  return (
    <div>
      ...
      <div className="relative">
        <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-xs text-white/30">
          ₺
        </span>
        <input
          id={id}
...
export function ResultBox({
  ...
  return (
    <div className="mt-4 rounded-xl bg-white/5 p-4">
      ...
      <p className="mt-1 text-2xl font-bold tracking-tight text-[#b38c56]">
        {value}
      </p>
      {subValue ? (
        <p className="mt-1 text-xs text-white/35">{subValue}</p>
      ) : null}
    </div>
  );
}
