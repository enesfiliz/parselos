    ...
    <div className="flex w-[min(82vw,280px)] shrink-0 snap-start flex-col md:w-[min(100%,292px)]">
      ...
      <div
        ...
        className={cn(
          ...
          isOver && "border-parsel-gold/30 bg-parsel-sunken",
        )}
      >
...
export function DealsKanbanBoard({
  ...
  useEffect(() => {
    if (isDraggingBoard || hasInFlightStageMutations(stageMutationMetaRef.current)) {
      return;
    }
    ...
    timers.set(
      ...
      setTimeout(() => {
        ...
        startTransition(async () => {
          const snapshot = dealsRef.current.find((item) => item.id === deal.id);
          const payload = dealPayloadWithLatestStage(deal, dealsRef.current);
          const response = await saveDealCard(payload);
          if (!response.success) {
            toast.error(response.error);
            if (snapshot) {
              commitDeals(
                dealsRef.current.map((item) =>
                  item.id === deal.id ? snapshot : item,
                ),
              );
              setSelectedDeal((current) =>
                current?.id === deal.id ? snapshot : current,
              );
            }
            return;
          }

          const live = dealsRef.current.find(
            (item) => item.id === response.data.id,
          );
          const merged = live
            ? mergeSavedDealWithLocalStage(live, response.data)
            : response.data;
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (useMock || isMockDealId(dealId)) {
      finishStageMutation(stageMutationMetaRef.current, dealId, mutationVersion);
      return;
    }
    ...
    void (async () => {
      ...
      if (merged) {
        commitDeals(applyStageSuccessMerge(dealsRef.current, merged));
      }
      finishStageMutation(
        stageMutationMetaRef.current,
        dealId,
